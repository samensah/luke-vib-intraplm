...
def matrixAlphaDivergence(Kx, Ky, Kxy, alpha, normalize=False):
    ...
    B = torch.matmul(torch.pow(mex, (alpha - 1))[None, :],
                     torch.matmul(M, torch.pow(mey, -alpha)[:, None]))
...
https://github.com/uk-cliplab/repMutualInformation/blob/main/ITL_utils.py
"""
...
def vonNeumannEigenValues(Ev, lowRank=False, rank_proportion=0.9):
    ...
    if lowRank:
        ...
        Ev_lr[n_eig:] = torch.mean(Ev[n_eig:])  # make equal the last n_eig
