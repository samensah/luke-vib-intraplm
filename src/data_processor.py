...
class Processor:
    ...
    def tokenize(self, tokens, subj_type, obj_type, ss, se, os, oe):
        ...
        for i_t, token in enumerate(tokens):
            ...
            if ss == None or se == None or os == None or oe == None:
        ...
        try:
            ...
            return input_ids, None, None, [0] + is_entity
...
class TACREDProcessor(Processor):
    def __init__(self, args, tokenizer):
        ...
        # train = json.load(open(args.data_dir + '/train.json', 'r', encoding='utf-8'))
        ...
        rels = sorted({item['relation'] for item in all_data})
        self.num_relations = len(rels)
        
        self.LABEL_TO_ID = {r: i for i, r in enumerate(rels)}
        # print(self.LABEL_TO_ID, len(self.LABEL_TO_ID))
    ...
    def read(self, file_in):
        ...
        for d in tqdm(data, desc=f"Processing {file_in}"):
            # Check if relation exists in LABEL_TO_ID
            if d['relation'] not in self.LABEL_TO_ID:
                ignored_relations.add(d['relation'])
                ignored_count += 1
                continue
            ...
            input_ids, new_ss, new_os, entity_mask = self.tokenize(
                tokens, d['subj_type'], d['obj_type'], ss, se, os, oe
            )
            
            if new_os is None or new_ss is None:
                continue
            if new_ss >= 512 or new_os >= 512:
                continue
        ...
        # Log ignored relations if any
        if ignored_relations:
            print(f"\n Ignored {ignored_count} examples from {file_in} "
                f"with unknown relations:")
            for rel in sorted(ignored_relations):
                print(f"  - {rel}")
        
        print(f"✓ Loaded {len(features)} valid features from {file_in}")
