...
class LukeEncoder(nn.Module):
    ...
    def apply_vib(self, sequence_output, entity_mask, mu_layer, std_layer, beta, device):
        """Apply VIB transformation to sequence output"""
        import torch.nn.functional as F
        from torch.autograd import Variable
        import math
        
        std_emb = F.softplus(std_layer(sequence_output) - 5, beta=1)
        mu_emb = mu_layer(sequence_output)
        ...
        sequence_output = (sequence_output * (1 - entity_mask[:, :, None]) + 
                          sequence_output * entity_mask[:, :, None] * (1 - beta) + 
                          (mu_emb + eps * std_emb) * entity_mask[:, :, None] * beta)
        
        return sequence_output, vib_loss
    ...
    def forward(
        ...
        return_dict=True,
        entity_mask=None, # added additional args for vib
        mu_layer=None,
        std_layer=None,
        beta=None,
        vib_layer_idx=None,
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            if (mu_layer is not None and std_layer is not None and entity_mask is not None and 
                ...
                if (vib_layer_idx == i or 
                    (vib_layer_idx == -1 and i == len(self.layer) - 1)):
                    
                    word_hidden_states, vib_loss = self.apply_vib(
                        word_hidden_states, entity_mask, mu_layer, std_layer, beta, device
                    )
...
class LukeModel(LukePreTrainedModel):
    ...
    def forward(
        ...
        # # NEW VIB LOCATION - AFTER ENCODING
